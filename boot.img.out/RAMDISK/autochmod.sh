...
ROOT_AUTOTRIGGER_PATH=/sdcard/oem_log
ROOT_TRIGGER_PATH=/sdcard/oem_log/trigger
...
function PreprocessLog(){
    ...
    ROOT_SDCARD_LOG_PATH=/sdcard/oem_log/${DIRTIME}_current
...
function GetLogDir(){
    ...
    FreeSize=${sdcardFreeSize}
    sleep 1
    CURRENT_DIR=`/system/xbin/busybox find /sdcard/oem_log/ -name *current`
...
function CleanAll(){
    ...
    rm  -rf  /sdcard/oem_log
}

function tranfer(){
    LOGDATE=`date +%F`
    mkdir /sdcard/oem_log/$LOGDATE/tombstones
    cp -rf  /data/tombstones /sdcard/oem_log/$LOGDATE/tombstones
    bugreport > /sdcard/oem_log/$LOGDATE/bugreport.txt
}
...
case "$config" in
    ...
    "asserttransfer")
        ...
        ;;
    "main")
        OLD_DIR=`/system/xbin/busybox find /sdcard/oem_log/ -name *current`
        DIR_TMP=`echo $OLD_DIR | /system/xbin/busybox sed "s/_current//"`
        mv $OLD_DIR $DIR_TMP
        PreprocessLog
        Logcat
        ;;
